...
def getAllPowerFeatures(raw, params, freq_bands, channel_sets):
    '''
    Calculate all spectral power features for the given mne.Raw object.
    '''
    F = {}

    # Features from M1
    psds_mean_epochs_M1, freqs = psd_M1(params, raw, plot_psd_epochs=None)
    F1_M1 = featureFunc_bandRelativePower(psds_mean_epochs_M1, freqs, freq_bands['M1-HighBeta'],
                                          freq_bands['M1-Range'])
    F['1'] = (F1_M1, '')

    # Features from M2
    psds_mean_epochs_M2, freqs = psd_M2(params, raw, plot_psd_epochs=None)
    F2_M2 = featureFunc_bandAbsolutePower(psds_mean_epochs_M2, freqs, freq_bands['M2-Beta1+Beta2'],
                                          norm_and_log_density=True)
    F3_M2 = featureFunc_bandAbsolutePower(psds_mean_epochs_M2, freqs, freq_bands['M2-Beta3'],
                                          channels=channel_sets['WashU-Bipolar-Frontal'],
                                          norm_and_log_density=True)
    F['2'] = (F2_M2, 'LOG(μV2/Hz)')
    F['3'] = (F3_M2, 'LOG(μV2/Hz)')

    # Features from M3
    psds_mean_epochs_M3, freqs = psd_M3(params, raw, plot_psd_epochs=None)
    n_eeg_channels = len(raw.copy().pick_types(eeg=True).ch_names)

    F4_M3 = []; F5_M3 = []; F6_M3 = []; F7_M3 = []; F8_M3 = []; F9_M3 = []
    F10_M3 = []; F11_M3 = []; F12_M3 = []; F13_M3 = []; F14_M3 = []
    for ich in np.arange(n_eeg_channels):
        F4_M3.append(featureFunc_bandAbsolutePower(psds_mean_epochs_M3, freqs, freq_bands['M3-Delta'],
                                                   channels=[ich], compute_magnitude=True))
        F5_M3.append(featureFunc_bandAbsolutePower(psds_mean_epochs_M3, freqs, freq_bands['M3-Theta'],
                                                   channels=[ich], compute_magnitude=True))
        F6_M3.append(featureFunc_bandAbsolutePower(psds_mean_epochs_M3, freqs, freq_bands['M3-Alpha'],
                                                   channels=[ich], compute_magnitude=True))
        F7_M3.append(featureFunc_bandAbsolutePower(psds_mean_epochs_M3, freqs, freq_bands['M3-Beta1'],
                                                   channels=[ich], compute_magnitude=True))
        F8_M3.append(featureFunc_bandAbsolutePower(psds_mean_epochs_M3, freqs, freq_bands['M3-Beta3'],
                                                   channels=[ich], compute_magnitude=True))
        F9_M3.append(featureFunc_bandRelativePower(psds_mean_epochs_M3, freqs, freq_bands['M3-Delta'],
                                                   freq_bands['M3-Range'], channels=[ich]))
        F10_M3.append(featureFunc_bandRelativePower(psds_mean_epochs_M3, freqs, freq_bands['M3-Theta'],
                                                    freq_bands['M3-Range'], channels=[ich]))
        F11_M3.append(featureFunc_bandRelativePower(psds_mean_epochs_M3, freqs, freq_bands['M3-Alpha'],
                                                    freq_bands['M3-Range'], channels=[ich]))
        F12_M3.append(featureFunc_bandRelativePower(psds_mean_epochs_M3, freqs, freq_bands['M3-Beta1'],
                                                    freq_bands['M3-Range'], channels=[ich]))
        F13_M3.append(featureFunc_bandRelativePower(psds_mean_epochs_M3, freqs, freq_bands['M3-Beta3'],
                                                    freq_bands['M3-Range'], channels=[ich]))
        F14_M3.append(featureFunc_bandAbsolutePower(psds_mean_epochs_M3, freqs, freq_bands['M3-Range'],
                                                    channels=[ich]))
    ...
    # Features from M4
    psds_mean_epochs_M4, freqs = psd_M4(params, raw, plot_psd_epochs=None)
    F15_M4 = featureFunc_bandAbsolutePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Range'])
    F16_M4 = featureFunc_bandAbsolutePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Band1'])
    F17_M4 = featureFunc_bandAbsolutePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Band2'])
    F18_M4 = featureFunc_bandAbsolutePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Band3'])
    F19_M4 = featureFunc_bandAbsolutePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Band4'])
    F20_M4 = featureFunc_bandAbsolutePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Band5'])
    F21_M4 = featureFunc_bandAbsolutePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Band6'])
    F22_M4 = featureFunc_bandAbsolutePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Band7'])
    F23_M4 = featureFunc_bandAbsolutePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Band8'])
    F24_M4 = featureFunc_bandAbsolutePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Band9'])
    F25_M4 = featureFunc_bandAbsolutePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Delta+Theta'])
    F26_M4 = featureFunc_bandAbsolutePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Alpha'])
    F27_M4 = featureFunc_bandAbsolutePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Beta'])

    F28_M4 = featureFunc_bandRelativePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Band1'],
                                           freq_bands['M4-Range'])
    F29_M4 = featureFunc_bandRelativePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Band2'],
                                           freq_bands['M4-Range'])
    F30_M4 = featureFunc_bandRelativePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Band3'],
                                           freq_bands['M4-Range'])
    F31_M4 = featureFunc_bandRelativePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Band4'],
                                           freq_bands['M4-Range'])
    F32_M4 = featureFunc_bandRelativePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Band5'],
                                           freq_bands['M4-Range'])
    F33_M4 = featureFunc_bandRelativePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Band6'],
                                           freq_bands['M4-Range'])
    F34_M4 = featureFunc_bandRelativePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Band7'],
                                           freq_bands['M4-Range'])
    F35_M4 = featureFunc_bandRelativePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Band8'],
                                           freq_bands['M4-Range'])
    F36_M4 = featureFunc_bandRelativePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Band9'],
                                           freq_bands['M4-Range'])
    F37_M4 = featureFunc_bandRelativePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Delta+Theta'],
                                           freq_bands['M4-Range'])
    F38_M4 = featureFunc_bandRelativePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Alpha'],
                                           freq_bands['M4-Range'])
    F39_M4 = featureFunc_bandRelativePower(psds_mean_epochs_M4, freqs, freq_bands['M4-Beta'],
                                           freq_bands['M4-Range'])

    F40_M4, F41_M4, F42_M4 = featureFunc_dominantFrequency(psds_mean_epochs_M4, freqs,
                                                           freq_bands['M4-Range'])

    F43_M4, F47_M4 = featureFunc_centroidFrequencyBand(psds_mean_epochs_M4, freqs,
                                                       freq_bands['M4-Delta+Theta'])
    F44_M4, F48_M4 = featureFunc_centroidFrequencyBand(psds_mean_epochs_M4, freqs,
                                                       freq_bands['M4-Alpha'])
    F45_M4, F49_M4 = featureFunc_centroidFrequencyBand(psds_mean_epochs_M4, freqs,
                                                       freq_bands['M4-Beta'])
    F46_M4, F50_M4 = featureFunc_centroidFrequencyBand(psds_mean_epochs_M4, freqs,
                                                       freq_bands['M4-Range'])
...
def featureFunc_centroidFrequencyBand(psds, freqs, band, channels=None, plot_psd=None):
    ...
    sigma : scalar
        The standard deviation of the centroid frequency.
    '''
...
def _computeFFTPSD(X, Fs, times=None, detrend=None, baseline=None, window=None,
    ...
    # Baseline correction
    if baseline and baseline is not None:
        # Baseline correction
