...
def make_mne_raw_from_imotions_and_bs_dig_data(
    ...
    fpath_report = os.path.join(
        results_dir, "Logs/DataImportToMNE", "importreport_%s_ses-%s.html" % (sub, ses)
    )
    ...
    if create_mne_report:
        report = mne.Report(title=sub, verbose=False)
    ...
    fpath_imotions_data = os.path.join(
        data_dir, sub, "ses-" + ses, "eeg", fname_imotions_data
    )
    ...
    if verbose:
        print("\n!!", message, "\n")
    if create_mne_report:
        report.add_html(title="Blink data import", html=message)
    ...
    if col_trigs in df_imotions.columns:
        try:
            trigger_times, trigger_desc = read_event_markers_from_imotions_data(
                df_imotions,
                df_eeg["Timestamp"][0],
                t_win_dupl_markers=t_win_dupl_markers,
                marker_col=col_trigs,
            )
        ...
        message = (
            "Trigger column ('%s') not found in raw data from iMotions." % col_trigs
        )
        try:
    ...
    if verbose:
        print("\n!!", message, "\n")
    if create_mne_report:
        report.add_html(title="Event data import", html=message)
    ...
    raw = convert_df_eeg_to_mne_raw_object(
        ...
        sfreq,
        report=report,
        create_mne_report=create_mne_report,
    )
    ...
    if create_mne_report and not create_plots:
        plt.close("all")  # closes all plt figures

    # Export raw data in FIF format
    fpath_exportedraw = os.path.join(
        data_dir, sub, "ses-" + ses, "eeg", fname_exportedraw
    )
...
def create_MNE_montage_from_BS_dig_data_new(fpath_dig):
    ...
    ch_pos = {
        u[0]: v.flatten()
        for u, v in zip(df_channel_locs["Name"], df_channel_locs["Loc"])
    }
    ...
    nasion = np.average(
        [loc for loc, lbl in zip(locs_hp, lbls_hp) if lbl == "NAS"], axis=0
    )
    lpa = np.average(
        [loc for loc, lbl in zip(locs_hp, lbls_hp) if lbl == "LPA"], axis=0
    )
    rpa = np.average(
        [loc for loc, lbl in zip(locs_hp, lbls_hp) if lbl == "RPA"], axis=0
    )
    hsp = np.array([loc for loc, lbl in zip(locs_hp, lbls_hp) if lbl == "EXTRA"])

    # Create montage
    dig = mne.channels.make_dig_montage(
        ch_pos=ch_pos, nasion=nasion, lpa=lpa, rpa=rpa, hsp=hsp
    )
...
def create_MNE_montage_from_BS_dig_data_old(fpath_dig):
    ...
    locs_hp = [w for w in df_headpoint_locs["Loc"][0].transpose()]
    nasion = np.average(
        [loc for loc, lbl in zip(locs_hp, lbls_hp) if lbl == "NA"], axis=0
    )
    lpa = np.average(
        [loc for loc, lbl in zip(locs_hp, lbls_hp) if lbl == "LPA"], axis=0
    )
...
def read_event_timings_from_imotions_data(
    df_imotions, event_col, positive_event_val, t_0
):
    df_event = df_imotions.drop(
        df_imotions.index[df_imotions[event_col].isna()]
    )  # drop rows without event data
    if len(df_event[event_col]) == 0:
        raise CustomError("No data found in %s column." % event_col)
    df_event.dropna(
        axis=1, how="all", inplace=True
    )  # drop columns that don't have any data left
    df_event = df_event.drop(
        df_event.index[df_event[event_col] != positive_event_val]
    )  # drop rows with negative event value (e.g., '0')
...
def read_event_markers_from_imotions_data(
    df_imotions, t_0, t_win_dupl_markers=1, marker_col="Description"
):
    """
    ...
    """
    df_triggers = df_imotions.drop(
        df_imotions.index[df_imotions[marker_col].isna()]
    )  # drop rows without trigger data
    df_triggers.dropna(
        axis=1, how="all", inplace=True
    )  # drop columns that don't have any data left
    df_triggers = df_triggers[
        ["Timestamp", marker_col]
    ]  # keep only trigger vals and their times

    # Drop rows for "trials-start" triggers
    df_triggers = df_triggers.drop(
        df_triggers.index[df_triggers[marker_col] == "trials-start"]
    )
    ...
    keep_inds = [0] + [
        (i + 1) for i, w in enumerate(tdiff) if w > t_win_dupl_markers
    ]  # jumps in tdiff
...
def add_interpolated_ecg_to_eeg(df_imotions, col_ecg, df_eeg, interp="linear"):
    ...
    # Extract ECG data
    df_ecg = df_imotions.drop(
        df_imotions.index[df_imotions[col_ecg].isna()]
    )  # drop rows without ECG data
    df_ecg.dropna(
        axis=1, how="all", inplace=True
    )  # drop columns that don't have any data left
    df_ecg = df_ecg[
        ["Timestamp", col_ecg]
    ]  # keep only primary ECG channel and the timestamps
    ...
    if interp == "linear":
        ...
        bspl = make_interp_spline(
            df_ecg["Timestamp"], df_ecg[col_ecg], k=3
        )  # B-spline cubic
...
def read_event_times_from_task(fpath_stimtime_task, recording_dt):
    """
    ...
    df_stimtime_task = df_stimtime_task.drop(
        df_stimtime_task.index[df_stimtime_task["EventName"] == "trials-start"]
    )
    ...
    task_stims_dt = [
        dt.datetime.fromtimestamp(ts) for ts in df_stimtime_task["EventTime"]
    ]

    # Subtract recording datetime to get relative times of event stims
    task_event_times = [(w - recording_dt).total_seconds() for w in task_stims_dt]

    return task_event_times, task_event_names


def read_event_markers_from_event_files(event_files_dir, tasknames, recording_dt):
    """
